...
export function generatePlayoffBrackets(standings: TeamStanding[], currentWeek: number): PlayoffBracket {
  ...
  // Generate Wild Card games (Week 19) - only if regular season is complete
  if (currentWeek > 18) {
    // AFC: 2v7, 3v6, 4v5
  ...
  // Generate Divisional games (Week 20) - only if wild card games are complete
  if (currentWeek > 19) {
    // AFC: 1v(lowest remaining), (2v7 winner)v(3v6 winner)
    if (afcStandings.length >= 1) {
  ...
  // Generate Conference Championships (Week 21) - only if divisional games are complete
  if (currentWeek > 20) {
  ...
  // Generate Super Bowl (Week 22) - only if conference championships are complete
  if (currentWeek > 21) {
...
export function isRegularSeasonComplete(currentWeek: number): boolean {
